...
MyNetworkUDP::MyNetworkUDP() : udp_server(-1), server_port(0), remote_port(0), tx_buffer(0), tx_buffer_len(0), rx_buffer(0) {}

MyNetworkUDP::~MyNetworkUDP() {
  stop();
}

uint8_t MyNetworkUDP::begin(IPAddress address, uint16_t port) {
  stop();
...
uint8_t MyNetworkUDP::begin(uint16_t p) {
  return begin(IPAddress(), p);
}

uint8_t MyNetworkUDP::beginMulticast(IPAddress address, uint16_t port) {
    if (!begin(IPAddress(), port)) {
        return 0;
    }
    ...
    if (!ip_addr_ismulticast(&addr)) {
        return 0; // keine Multicast-Adresse
    }

#if LWIP_IPV6
    if (address.type() == IPv6) {
        // optional: IPv6 IGMP join, unverändert
        ...
        for (netif *intf = netif_list; intf != nullptr; intf = intf->next) {
            ...
            if (intf->name[0] != 'l' || intf->name[1] != 'o') { // skip loopback
        ...
        if (!joined) {
            log_e("could not join IPv6 multicast: %d", errno);
            stop();
            return 0;
        }
    } else
#endif
    {
        ...
        if (setsockopt(udp_server, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
            log_e("could not join IPv4 multicast: %d", errno);
            stop();
            return 0;
        }

        // === ESP32 LWIP: automatisch periodische IGMP Reports aktivieren ===
#ifdef ARDUINO_ARCH_ESP32
        for (struct netif *intf = netif_list; intf != nullptr; intf = intf->next) {
            if (intf->name[0] != 'l' || intf->name[1] != 'o') { // skip loopback "lo"
                intf->flags |= NETIF_FLAG_IGMP;
                log_d("NETIF_FLAG_IGMP set on netif %c%c", intf->name[0], intf->name[1]);
            }
        }
#endif
    }
...
int MyNetworkUDP::beginMulticastPacket() {
...
int MyNetworkUDP::beginPacket(IPAddress ip, uint16_t port) {
  remote_ip = ip;
  remote_port = port;
  return beginPacket();
}

int MyNetworkUDP::beginPacket(const char *host, uint16_t port) {
...
size_t MyNetworkUDP::write(uint8_t data) {
...
size_t MyNetworkUDP::write(const uint8_t *buffer, size_t size) {
  size_t i;
  for (i = 0; i < size; i++) {
    write(buffer[i]);
  }
...
void MyNetworkUDP::flush() {
  clear();
}

int MyNetworkUDP::parsePacket() {
  if (rx_buffer) {
    return 0;
  }
...
int MyNetworkUDP::read(unsigned char *buffer, size_t len) {
  return read((char *)buffer, len);
}

int MyNetworkUDP::read(char *buffer, size_t len) {
  if (!rx_buffer) {
    return 0;
  }
...
IPAddress MyNetworkUDP::remoteIP() {
  return remote_ip;
}

uint16_t MyNetworkUDP::remotePort() {
  return remote_port;
}

void MyNetworkUDP::test() {
